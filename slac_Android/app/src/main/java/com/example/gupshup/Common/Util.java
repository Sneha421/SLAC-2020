...
public class Util {
    ...
    public static  void updateDeviceToken(final Context context, String token)
    {
        ...
        if(currentUser!=null) {
            ...
            DatabaseReference databaseReference = rootRef.child(NodesList.TOKENS).child(currentUser.getUid());

            HashMap<String, String> hashMap = new HashMap<>();
            hashMap.put(NodesList.DEVICE_TOKEN, token);
            ...
            databaseReference.setValue(hashMap).addOnCompleteListener(new OnCompleteListener<Void>() {
                ...
                public void onComplete(@NonNull Task<Void> task) {
                    ...
                    {
                        Toast.makeText(context, context.getString(R.string.failed_to_save_device_token), Toast.LENGTH_SHORT).show();
                    }
                }
            });
    ...
    {
        DatabaseReference rootRef = FirebaseDatabase.getInstance().getReference();
        DatabaseReference databaseReference = rootRef.child(NodesList.TOKENS).child(userId);

        databaseReference.addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot dataSnapshot)
            {
                if(dataSnapshot.child(NodesList.DEVICE_TOKEN).getValue()!=null)
                {

                }
                String deviceToken = dataSnapshot.child(NodesList.DEVICE_TOKEN).getValue().toString();
    ...
    public static  void updateChatDetails(final Context context, final String currentUserId, final String chatUserId, final String lastMessage)
    {
        ...
        DatabaseReference chatRef = rootRef.child(NodesList.CHATS).child(chatUserId).child(currentUserId);

        chatRef.addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                String currentCount="0";
                if(dataSnapshot.child(NodesList.UNREAD_COUNT).getValue()!=null)
                    currentCount = dataSnapshot.child(NodesList.UNREAD_COUNT).getValue().toString();


                Map chatMap = new HashMap();
                chatMap.put(NodesList.TIME_STAMP, ServerValue.TIMESTAMP);
                chatMap.put(NodesList.UNREAD_COUNT, Integer.valueOf(currentCount)+1);
                chatMap.put(NodesList.LAST_MESSAGE, lastMessage);
                chatMap.put(NodesList.LAST_MESSAGE_TIME, ServerValue.TIMESTAMP);

                Map chatUserMap = new HashMap();
                chatUserMap.put(NodesList.CHATS +"/" + chatUserId + "/" + currentUserId, chatMap);
